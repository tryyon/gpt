...
export function ColorSelector({ onColorSelect, selectedColor, isOpen, onClose }: ColorSelectorProps) {
  ...
  const filteredColors = useMemo(() => {
    ...
    const results: { family: string; shades: Array<{ name: string; hex: string; id: string }> }[] = [];
    
    Object.entries(colorFamilies).forEach(([family, shades]) => {
      const matchingShades = (shades as ReadonlyArray<{ name: string; hex: string; id: string }>).filter(shade => 
