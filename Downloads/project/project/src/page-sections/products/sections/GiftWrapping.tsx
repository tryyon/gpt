    ...
    <Box>
      <Paper variant="outlined" sx={{ p: 3 }}>
        ...
        <Controller
          name="attributes"
          control={control}
          defaultValue={{ material: '', size: '', color: '' }}
          render={({ field }) => (
            <FormControl component="fieldset" error={!!(errors.attributes as any)?.giftWrapping} fullWidth>
              <RadioGroup
                {...field}
                onChange={(e) => {
                  field.onChange(e);
              ...
              {errors?.attributes && (errors.attributes as any)?.giftWrapping && (
                <FormHelperText>{(errors.attributes as any).giftWrapping.message}</FormHelperText>
              )}
            </FormControl>
          )}
        />
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <FormControlLabel
                            control={
                              <Radio
                                checked={Array.isArray(field.value) ? field.value.includes(option.id) : false}
                                onChange={(e) => {
