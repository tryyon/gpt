...
export function ModifierOptions({ form }: ModifierOptionsProps) {
  ...
  const [modifierOptions, setModifierOptions] = useState<ModifierOption[]>([initialModifierOption]);
  ...
  const handleSaveModifier = () => {
    ...
    const optionsWithNumericPrices = modifierOptions.map(option => ({
      ...option,
      priceAdjustment: option.priceAdjustment === '' ? 0 : parseFloat(String(option.priceAdjustment)),
    }));
    
    if (isEditMode && currentModifier) {
      setModifiers(prev =>
        prev.map(modifier =>
          ...
          <Box sx={{ pt: 2 }}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                ...
                {/* FIX: Map over modifierOptions to render each option input */}
