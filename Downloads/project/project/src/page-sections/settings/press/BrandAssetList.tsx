...
import { useState } from 'react';
 import { BrandAsset } from '@/lib/validations/content';
 // If BrandAsset is a default export:


// Or, if it has a different named export, use the correct name:
// import type { CorrectExportName } from '@/lib/validations/content';
...
export function BrandAssetList({ assets, onEdit, onDelete }: BrandAssetListProps) {
  ...
  const groupedAssets: Record<string, BrandAsset[]> = assets.reduce((acc: Record<string, BrandAsset[]>, asset) => {
    if (!acc[asset.type]) {
      acc[asset.type] = [];
    }
      ...
      {Object.entries(groupedAssets).map(([type, typeAssets]) => (
        <Box key={type} sx={{ mb: 4 }}>
          ...
          <Grid container spacing={3}>
            {typeAssets.map((asset: BrandAsset) => (
              ...
              <Card variant="outlined">
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                  ...
                  {isPreviewable(asset.format) && (
                    ...
                    <IconButton
                      size="small"
                      onClick={(e: React.MouseEvent<HTMLButtonElement>) => handlePreview(asset, e)}
                    >
